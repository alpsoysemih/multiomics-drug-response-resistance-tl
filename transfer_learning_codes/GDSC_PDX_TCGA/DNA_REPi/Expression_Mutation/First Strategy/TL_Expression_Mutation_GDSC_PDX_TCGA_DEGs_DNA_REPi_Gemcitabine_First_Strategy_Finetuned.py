...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            load_models_from + 'Exprs_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy.pt')
        AutoencoderM = torch.load(
            load_models_from + 'Mut_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy.pt')
        Clas = torch.load(
            load_models_from + 'Class_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_DNA_REPi_Expression_Mutation_GDSC_PDX_TCGA_Gemcitabine_First_Strategy_Finetuned_{iters + 1}.pt')
