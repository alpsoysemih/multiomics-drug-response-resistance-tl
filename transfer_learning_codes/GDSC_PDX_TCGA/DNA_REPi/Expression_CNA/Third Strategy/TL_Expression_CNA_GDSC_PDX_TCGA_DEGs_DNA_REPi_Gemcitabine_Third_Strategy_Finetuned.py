...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            load_models_from + 'Exprs_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy.pt')
        AutoencoderC = torch.load(
            load_models_from + 'CNA_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy.pt')
        Clas = torch.load(
            load_models_from + 'Class_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_DNA_REPi_Expression_CNA_GDSC_PDX_TCGA_Gemcitabine_Third_Strategy_Finetuned_{iters + 1}.pt')
