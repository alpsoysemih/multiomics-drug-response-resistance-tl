...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_PDX/Expression_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_PDX/Expression_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_PDX/Expression_CNA/AUC and Cost Plots (Finetuned)/"
...
# fill NA values in GDSC CNA dataset (CYTOi)
...
# select shared samples between GDSC expression, mutation and CNA datasets (CYTOi)
ls2 = GDSCE.index.intersection(GDSCC.index)
ls2 = ls2.intersection(GDSCR.index)


# subset shared genes and samples in GDSC expression, CNA and response datasets (CYTOi)
...
# assign GDSC expression, CNA and response datasets (CYTOi) to new variables
exprs_z = GDSCE
cna = GDSCC
responses = GDSCR


# select drugs in GDSC response dataset (CYTOi)
drugs = set(responses["drug"].values)
print("Drugs:", drugs)


# subset GDSC expression and CNA datasets (CYTOi) as to drugs
expression_zscores = []
CNA = []
for drug in drugs:
...
# filter shared samples between the subsetted GDSC expression and CNA datasets (CYTOi) 
ls5 = GDSCEv2.index.intersection(GDSCCv2.index)


# subset shared genes and samples in the subsetted GDSC expression, CNA, and response datasets (CYTOi) 
...
# read PDX expression dataset (Paclitaxel)
pdx_expr_data = pd.read_excel(pdx_data_dir + "/nm.3954-S2.xlsx",
                              "PCT raw data")
pdx_expr_data = pdx_expr_data.loc[pdx_expr_data["Treatment"] == "paclitaxel",]
pdx_expr_data = pdx_expr_data.loc[:,["Model","Tumor Type"]]
pdx_expr_data = pdx_expr_data.drop_duplicates()
pdx_expr_data.set_index("Model", inplace = True) 


# merge PDX expression and response datasets (Paclitaxel)
# find PDX cohorts with their sample counts
dat_pdx = pd.merge(pdx_expr_data,
                   PDXRpaclitaxel,
                   left_index = True,
                   right_index = True)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Paclitaxel_GDSC_PDX_CYTOi_Expression_CNA_Finetuned_{iters + 1}.pt')
