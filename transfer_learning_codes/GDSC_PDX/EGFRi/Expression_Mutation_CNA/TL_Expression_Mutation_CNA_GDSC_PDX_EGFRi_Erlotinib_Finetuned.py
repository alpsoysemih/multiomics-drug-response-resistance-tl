...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_PDX/Expression_Mutation_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_PDX/Expression_Mutation_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_PDX/Expression_Mutation_CNA/AUC and Cost Plots (Finetuned)/"
...
GDSCE = pd.read_csv(dataset_dir + "GDSC_exprs.z.EGFRi.tsv",
                    sep="\t",
                    index_col=0,
                    decimal=",")
...
GDSCM = pd.read_csv(dataset_dir + "GDSC_mutations.EGFRi.tsv",
...
GDSCC = pd.read_csv(dataset_dir + "GDSC_CNA.EGFRi.tsv",
                    sep="\t", index_col=0, decimal=".")
...
GDSCR = pd.read_csv(dataset_dir + "GDSC_response.EGFRi.tsv",
                    sep="\t",
                    index_col=0,
                    decimal=",")
...
# Read PDX expression dataset (Erlotinib)
PDXEerlotinib = pd.read_csv(dataset_dir + "PDX_exprs.Erlotinib.eb_with.GDSC_exprs.Erlotinib.tsv",
                      sep="\t", index_col=0, decimal=",")
PDXEerlotinib = pd.DataFrame.transpose(PDXEerlotinib)


# Read PDX mutation dataset (Erlotinib)
PDXMerlotinib = pd.read_csv(dataset_dir + "PDX_mutations.Erlotinib.tsv",
                      sep="\t", index_col=0, decimal=",")
PDXMerlotinib.drop_duplicates(keep='last')
PDXMerlotinib = pd.DataFrame.transpose(PDXMerlotinib)
PDXMerlotinib = PDXMerlotinib.loc[:, ~PDXMerlotinib.columns.duplicated()]


# Read PDX CNA dataset (Erlotinib)
PDXCerlotinib = pd.read_csv(dataset_dir + "PDX_CNA.Erlotinib.tsv",
                      sep="\t", index_col=0, decimal=",")
...
ls = ls.intersection(PDXMerlotinib.columns)
...
# select shared samples between PDX expression, mutation, and CNA datasets (Erlotinib)
ls3 = PDXEerlotinib.index.intersection(PDXMerlotinib.index)
ls3 = ls3.intersection(PDXCerlotinib.index)


# subset shared samples and genes in PDX expression, mutation, and CNA datasets (Erlotinib)
PDXEerlotinib = PDXEerlotinib.loc[ls3, ls]
PDXMerlotinib = PDXMerlotinib.loc[ls3, ls]
PDXCerlotinib = PDXCerlotinib.loc[ls3, ls]



# assign GDSC expression, mutation, CNA, and response datasets (EGFRi) to new variables
exprs_z = GDSCE
mut = GDSCM
cna = GDSCC
responses = GDSCR
...
# list drugs in GDSC response dataset (EGFRi)
...
PDXEerlotinib = PDXEerlotinib.loc[:,ls4]
PDXMerlotinib = PDXMerlotinib.loc[:,ls4]
PDXCerlotinib = PDXCerlotinib.loc[:,ls4]
...
print(f"PDX # of common samples and genes for Erlotinib (Mutation): {PDXMerlotinib.shape}\n")
...
PDXRerlotinib = pd.read_csv(dataset_dir + "PDX_response.Erlotinib.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
# merge PDX expression and response datasets (Erlotinib)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA.pt')
        ...
        title = 'Cost Erlotinib iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
        ...
        title = 'AUC Erlotinib iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
            format(iters, k, mbs, hdm1, hdm2, hdm3, lre, lrm, lrc,
                   epch, rate1, rate2, rate3, rate4, wd, lrCL)
    ...
    torch.save(AutoencoderE,
               f"Exprs_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Erlotinib_GDSC_PDX_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
