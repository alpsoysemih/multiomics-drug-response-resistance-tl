...
PDXMcetuximab = pd.read_csv(dataset_dir + "PDX_mutations.Cetuximab.tsv",
                      sep="\t", index_col=0, decimal=",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation.pt')
        Clas = torch.load(
            save_models_to + 'Class_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Cetuximab_GDSC_PDX_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt')
