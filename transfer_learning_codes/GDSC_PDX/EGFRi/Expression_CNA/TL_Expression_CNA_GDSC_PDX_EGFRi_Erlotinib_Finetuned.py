...
# subset shared samples and genes in PDX expression and CNA datasets (Erlotinib)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Erlotinib_GDSC_PDX_EGFRi_Expression_CNA_Finetuned_{iters + 1}.pt')
