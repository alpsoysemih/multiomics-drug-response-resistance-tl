...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt')
