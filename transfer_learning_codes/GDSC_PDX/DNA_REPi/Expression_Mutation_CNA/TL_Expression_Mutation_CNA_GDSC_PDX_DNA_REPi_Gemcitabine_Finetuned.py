...
PDXEgemcitabine = pd.read_csv(dataset_dir + "PDX_exprs.Gemcitabine.eb_with.GDSC_exprs.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
PDXEgemcitabine = pd.DataFrame.transpose(PDXEgemcitabine)


# Read PDX mutation dataset (Gemcitabine)
PDXMgemcitabine = pd.read_csv(dataset_dir + "PDX_mutations.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
...
PDXCgemcitabine = pd.read_csv(dataset_dir + "PDX_CNA.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
...
PDXRgemcitabine = pd.read_csv(dataset_dir + "PDX_response.Gemcitabine.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
# merge PDX expression and response datasets (Gemcitabine)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Gemcitabine_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
