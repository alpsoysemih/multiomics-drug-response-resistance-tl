...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_PDX/Expression_Mutation_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_PDX/Expression_Mutation_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_PDX/Expression_Mutation_CNA/AUC and Cost Plots (Finetuned)/"
...
# Read PDX expression dataset (5-Fluorouracil)
PDXEfluorouracil = pd.read_csv(dataset_dir + "PDX_exprs.5-Fluorouracil.eb_with.GDSC_exprs.5-Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
PDXEfluorouracil = pd.DataFrame.transpose(PDXEfluorouracil)


# Read PDX mutation dataset (5-Fluorouracil)
PDXMfluorouracil = pd.read_csv(dataset_dir + "PDX_mutations.5-Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
...
PDXCfluorouracil = pd.read_csv(dataset_dir + "PDX_CNA.5-Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
...
# subset shared samples and genes in PDX expression, mutation, and CNA datasets (5-Fluorouracil)
...
PDXMfluorouracil = PDXMfluorouracil.loc[:,ls4]
...
PDXRfluorouracil = pd.read_csv(dataset_dir + "PDX_response.5-Fluorouracil.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
print(f"There are {Ytsfluorouracil.shape[0]} samples with response data in the PDX (5-Fluorouracil)\n")


# read PDX cohorts (5-Fluorouracil)
...
# merge PDX expression and response datasets (5-Fluorouracil)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Fluorouracil_GDSC_PDX_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
