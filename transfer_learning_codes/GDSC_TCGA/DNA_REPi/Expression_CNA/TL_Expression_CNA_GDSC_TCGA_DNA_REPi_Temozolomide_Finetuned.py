...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_TCGA/Expression_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_TCGA/Expression_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_TCGA/Expression_CNA/AUC and Cost Plots (Finetuned)/"
...
TCGACtemozolomide = pd.read_csv("TCGA_CNA.Temozolomide.tsv",
                      sep="\t", index_col=0, decimal=",")
...
# fill NA values in GDSC CNA dataset (DNA_REPi)
...
# select shared samples between GDSC expression, mutation and CNA datasets (DNA_REPi)
...
# select shared samples between TCGA expression and CNA datasets (Temozolomide)
ls3 = TCGAEtemozolomide.index.intersection(TCGACtemozolomide.index)


# subset shared samples and genes in TCGA expression and CNA datasets (Temozolomide)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
        ...
        title = 'Cost Temozolomide iter = {}, fold = {}, mb_size = {},  h_dim[1,2] = ({},{}), \nlr[E,C] = ({}, {}), epoch = {}, rate[1,2,3] = ({},{},{}), wd = {}, lrCL = {}'.\
        ...
        title = 'AUC Temozolomide iter = {}, fold = {}, mb_size = {},  h_dim[1,2] = ({},{}), \nlr[E, C] = ({}, {}), epoch = {}, rate[1,2,3] = ({},{},{}), wd = {}, lrCL = {}'.\
            format(iters, k, mbs, hdm1, hdm2, lre, lrc,
                   epch, rate1, rate2, rate3, wd, lrCL)
    ...
    torch.save(AutoencoderE,
               f"Exprs_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt')
