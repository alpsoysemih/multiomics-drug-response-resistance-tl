...
# subset shared samples and genes in TCGA expression and CNA datasets (5-Fluorouracil)
...
print(f"TCGA # of common samples and genes for fluorouracil (Expression): {TCGAEfluorouracil.shape}")
print(f"TCGA # of common samples and genes for fluorouracil (CNA): {TCGACfluorouracil.shape}\n")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_CNA_Finetuned_{iters + 1}.pt')
