...
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_TCGA/Expression_Mutation_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/DNA_REPi/save_models/GDSC_TCGA/Expression_Mutation_CNA/AUC and Cost Plots (Finetuned)/"
...
GDSCE = pd.read_csv(dataset_dir + "GDSC_exprs.z.DNA_REPi.tsv",
                    sep="\t",
                    index_col=0,
                    decimal=",")
...
GDSCM = pd.read_csv(dataset_dir + "GDSC_mutations.DNA_REPi.tsv",
...
GDSCC = pd.read_csv(dataset_dir + "GDSC_CNA.DNA_REPi.tsv",
                    sep="\t", index_col=0, decimal=".")
...
GDSCR = pd.read_csv(dataset_dir + "GDSC_response.DNA_REPi.tsv",
                    sep="\t",
                    index_col=0,
                    decimal=",")
...
# Read TCGA expression dataset (Temozolomide)
TCGAEtemozolomide = pd.read_csv(dataset_dir + "TCGA_exprs.Temozolomide.eb_with.GDSC_exprs.Temozolomide.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEtemozolomide = pd.DataFrame.transpose(TCGAEtemozolomide)


# Read TCGA mutation dataset (Temozolomide)
TCGAMtemozolomide = pd.read_csv(dataset_dir + "TCGA_mutations.Temozolomide.tsv",
                      sep="\t", index_col=0, decimal=",")
...
# Read TCGA CNA dataset (Temozolomide)
TCGACtemozolomide = pd.read_csv(dataset_dir + "TCGA_CNA.Temozolomide.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGACtemozolomide.drop_duplicates(keep='last')
TCGACtemozolomide = pd.DataFrame.transpose(TCGACtemozolomide)
TCGACtemozolomide = TCGACtemozolomide.loc[:, ~TCGACtemozolomide.columns.duplicated()]
...
ls = ls.intersection(TCGACtemozolomide.columns)
ls = pd.unique(ls)
...
# select shared samples between TCGA expression, mutation, and CNA datasets (Temozolomide)
ls3 = TCGAEtemozolomide.index.intersection(TCGAMtemozolomide.index)
ls3 = ls3.intersection(TCGACtemozolomide.index)


# subset shared samples and genes in TCGA expression, mutation, and CNA datasets (Temozolomide)
TCGAEtemozolomide = TCGAEtemozolomide.loc[ls3, ls]
TCGAMtemozolomide = TCGAMtemozolomide.loc[ls3, ls]
TCGACtemozolomide = TCGACtemozolomide.loc[ls3, ls]
...
# list drugs in GDSC response dataset (DNA_REPi)
...
TCGACtemozolomide = TCGACtemozolomide.loc[:,ls4]
...
print(f"TCGA # of common samples and genes for Temozolomide (CNA): {TCGACtemozolomide.shape}\n")
...
TCGARtemozolomide = pd.read_csv(dataset_dir + "TCGA_response.Temozolomide.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        ...
        title = 'Cost Temozolomide iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
        ...
        title = 'AUC Temozolomide iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
            format(iters, k, mbs, hdm1, hdm2, hdm3, lre, lrm, lrc,
                   epch, rate1, rate2, rate3, rate4, wd, lrCL)
    ...
    torch.save(AutoencoderE,
               f"Exprs_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Temozolomide_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
