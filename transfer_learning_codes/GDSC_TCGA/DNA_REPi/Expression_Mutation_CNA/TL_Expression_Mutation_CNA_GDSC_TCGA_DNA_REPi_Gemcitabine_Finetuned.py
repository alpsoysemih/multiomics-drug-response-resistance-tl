...
TCGAEgemcitabine = pd.read_csv(dataset_dir + "TCGA_exprs.Gemcitabine.eb_with.GDSC_exprs.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEgemcitabine = pd.DataFrame.transpose(TCGAEgemcitabine)


# Read TCGA mutation dataset (Gemcitabine)
TCGAMgemcitabine = pd.read_csv(dataset_dir + "TCGA_mutations.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACgemcitabine = pd.read_csv(dataset_dir + "TCGA_CNA.Gemcitabine.tsv",
                      sep="\t", index_col=0, decimal=",")
...
ls3 = TCGAEgemcitabine.index.intersection(TCGAMgemcitabine.index)
ls3 = ls3.intersection(TCGACgemcitabine.index)
...
TCGAMgemcitabine = TCGAMgemcitabine.loc[ls3, ls]
...
TCGAMgemcitabine = TCGAMgemcitabine.loc[:,ls4]
...
TCGARgemcitabine = pd.read_csv(dataset_dir + "TCGA_response.Gemcitabine.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
Ytsgemcitabine = TCGARgemcitabine["response"].values    
...
# filter PAAD cohorts (Gemcitabine)
filter = (TCGARgemcitabine["cohort"] == "PAAD")
tcga_paad_samples = dat_tcga.loc[filter,].index


# filter PAAD cohort responses (Gemcitabine) 
Ytsgemcitabine_paad = TCGARgemcitabine.loc[tcga_paad_samples,"response"].values
print(TCGARgemcitabine.loc[tcga_paad_samples,"response"].value_counts())
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
