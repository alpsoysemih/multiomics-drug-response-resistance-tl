...
# Read TCGA expression dataset (Cisplatin)
TCGAEcisplatin = pd.read_csv(dataset_dir + "TCGA_exprs.Cisplatin.eb_with.GDSC_exprs.Cisplatin.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEcisplatin = pd.DataFrame.transpose(TCGAEcisplatin)


# Read TCGA mutation dataset (Cisplatin)
TCGAMcisplatin = pd.read_csv(dataset_dir + "TCGA_mutations.Cisplatin.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACcisplatin = pd.read_csv(dataset_dir + "TCGA_CNA.Cisplatin.tsv",
                      sep="\t", index_col=0, decimal=",")
...
# subset shared samples and genes in TCGA expression, mutation, and CNA datasets (Cisplatin)
...
TCGARcisplatin = pd.read_csv(dataset_dir + "TCGA_response.Cisplatin.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        ...
        title = 'Cost Cisplatin iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
        ...
        title = 'AUC Cisplatin iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
            format(iters, k, mbs, hdm1, hdm2, hdm3, lre, lrm, lrc,
                   epch, rate1, rate2, rate3, rate4, wd, lrCL)
    ...
    torch.save(AutoencoderE,
               f"Exprs_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Cisplatin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
