...
TCGAEdoxorubicin = pd.read_csv(dataset_dir + "TCGA_exprs.Doxorubicin.eb_with.GDSC_exprs.Doxorubicin.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEdoxorubicin = pd.DataFrame.transpose(TCGAEdoxorubicin)


# Read TCGA mutation dataset (Doxorubicin)
TCGAMdoxorubicin = pd.read_csv(dataset_dir + "TCGA_mutations.Doxorubicin.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACdoxorubicin = pd.read_csv(dataset_dir + "TCGA_CNA.Doxorubicin.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACdoxorubicin = TCGACdoxorubicin.loc[:,ls4]
...
TCGARdoxorubicin = pd.read_csv(dataset_dir + "TCGA_response.Doxorubicin.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Doxorubicin_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
