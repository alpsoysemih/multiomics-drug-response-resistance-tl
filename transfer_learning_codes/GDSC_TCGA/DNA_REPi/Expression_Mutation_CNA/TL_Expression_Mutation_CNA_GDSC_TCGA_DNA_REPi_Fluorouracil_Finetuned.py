...
# Read TCGA expression dataset (5-Fluorouracil)
TCGAEfluorouracil = pd.read_csv(dataset_dir + "TCGA_exprs.Fluorouracil.eb_with.GDSC_exprs.5-Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEfluorouracil = pd.DataFrame.transpose(TCGAEfluorouracil)


# Read TCGA mutation dataset (5-Fluorouracil)
TCGAMfluorouracil = pd.read_csv(dataset_dir + "TCGA_mutations.Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACfluorouracil = pd.read_csv(dataset_dir + "TCGA_CNA.Fluorouracil.tsv",
                      sep="\t", index_col=0, decimal=",")
...
ls3 = TCGAEfluorouracil.index.intersection(TCGAMfluorouracil.index)
ls3 = ls3.intersection(TCGACfluorouracil.index)


# subset shared samples and genes in TCGA expression, mutation, and CNA datasets (5-Fluorouracil)
TCGAEfluorouracil = TCGAEfluorouracil.loc[ls3, ls]
TCGAMfluorouracil = TCGAMfluorouracil.loc[ls3, ls]
...
TCGAMfluorouracil = TCGAMfluorouracil.loc[:,ls4]
...
TCGARfluorouracil = pd.read_csv(dataset_dir + "TCGA_response.Fluorouracil.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA.pt')
        ...
        title = 'Cost Fluorouracil iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
        ...
        title = 'AUC Fluorouracil iter = {}, fold = {}, mb_size = {},  h_dim[1,2,3] = ({},{},{}), \nlr[E,M,C] = ({}, {},{}), epoch = {}, rate[1,2,3,4] = ({},{},{},{}), wd = {}, lrCL = {}'.\
            format(iters, k, mbs, hdm1, hdm2, hdm3, lre, lrm, lrc,
                   epch, rate1, rate2, rate3, rate4, wd, lrCL)
    ...
    torch.save(AutoencoderE,
               f"Exprs_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Fluorouracil_GDSC_TCGA_DNA_REPi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
