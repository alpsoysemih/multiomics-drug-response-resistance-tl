...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation.pt')
        Clas = torch.load(
            save_models_to + 'Class_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Gemcitabine_GDSC_TCGA_DNA_REPi_Expression_Mutation_Finetuned_{iters + 1}.pt')
