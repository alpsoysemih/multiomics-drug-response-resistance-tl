...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation_CNA/AUC and Cost Plots (Finetuned)/"
...
TCGACcetuximab = pd.read_csv(dataset_dir + "TCGA_CNA.Cetuximab.tsv",
                      sep="\t", index_col=0, decimal=",")
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
