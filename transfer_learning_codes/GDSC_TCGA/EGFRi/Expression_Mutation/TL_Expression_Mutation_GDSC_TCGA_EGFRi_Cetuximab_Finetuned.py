...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/EGFRi/save_models/GDSC_TCGA/Expression_Mutation/AUC and Cost Plots (Finetuned)/"
...
TCGAEcetuximab = pd.read_csv(dataset_dir + "TCGA_exprs.Cetuximab.eb_with.GDSC_exprs.Cetuximab.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEcetuximab = pd.DataFrame.transpose(TCGAEcetuximab)


# Read TCGA mutation dataset (Cetuximab)
TCGAMcetuximab = pd.read_csv(dataset_dir + "TCGA_mutations.Cetuximab.tsv",
                      sep="\t", index_col=0, decimal=",")
...
# subset shared samples and genes in TCGA expression and mutation datasets (Cetuximab)
...
TCGARcetuximab = pd.read_csv(dataset_dir + "TCGA_response.Cetuximab.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
# filter HNSC cohort responses (Cetuximab) 
...
print(TCGARcetuximab.loc[tcga_hnsc_samples,"response"].value_counts())
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation.pt')
        Clas = torch.load(
            save_models_to + 'Class_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Cetuximab_GDSC_TCGA_EGFRi_Expression_Mutation_Finetuned_{iters + 1}.pt')
