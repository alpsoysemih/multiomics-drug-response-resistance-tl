...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/MITOSISi/save_models/GDSC_TCGA/Expression_Mutation_CNA/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/MITOSISi/save_models/GDSC_TCGA/Expression_Mutation_CNA/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/MITOSISi/save_models/GDSC_TCGA/Expression_Mutation_CNA/AUC and Cost Plots (Finetuned)/"
...
GDSCC = pd.read_csv(dataset_dir + "GDSC_CNA.MITOSISi.tsv",
                    sep="\t", index_col=0, decimal=".")
...
TCGAEpaclitaxel = pd.read_csv(dataset_dir + "TCGA_exprs.Paclitaxel.eb_with.GDSC_exprs.Paclitaxel.tsv",
                      sep="\t", index_col=0, decimal=",")
TCGAEpaclitaxel = pd.DataFrame.transpose(TCGAEpaclitaxel)


# Read TCGA mutation dataset (Paclitaxel)
TCGAMpaclitaxel = pd.read_csv(dataset_dir + "TCGA_mutations.Paclitaxel.tsv",
                      sep="\t", index_col=0, decimal=",")
...
TCGACpaclitaxel = pd.read_csv(dataset_dir + "TCGA_CNA.Paclitaxel.tsv",
                      sep="\t", index_col=0, decimal=",")
...
ls3 = TCGAEpaclitaxel.index.intersection(TCGAMpaclitaxel.index)
ls3 = ls3.intersection(TCGACpaclitaxel.index)
...
TCGAMpaclitaxel = TCGAMpaclitaxel.loc[ls3, ls]
TCGACpaclitaxel = TCGACpaclitaxel.loc[ls3, ls]



# assign GDSC expression, mutation, CNA, and response datasets (MITOSISi) to new variables
exprs_z = GDSCE
mut = GDSCM
cna = GDSCC
responses = GDSCR
...
TCGAMpaclitaxel = TCGAMpaclitaxel.loc[:,ls4]
...
TCGARpaclitaxel = pd.read_csv(dataset_dir + "TCGA_response.Paclitaxel.tsv", 
                      sep = "\t", 
                      index_col=0, 
                      decimal = ",")
...
# filter BRCA cohorts (Paclitaxel)
...
print(TCGARpaclitaxel.loc[tcga_brca_samples,"response"].value_counts())
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt")    
    torch.save(Clas, f'Class_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_Mutation_CNA_Finetuned_{iters + 1}.pt')
