...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA.pt')
        AutoencoderC = torch.load(
            save_models_to + 'CNA_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA.pt')
        Clas = torch.load(
            save_models_to + 'Class_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderC, f"CNA_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Paclitaxel_GDSC_TCGA_MITOSISi_Expression_CNA_Finetuned_{iters + 1}.pt')
