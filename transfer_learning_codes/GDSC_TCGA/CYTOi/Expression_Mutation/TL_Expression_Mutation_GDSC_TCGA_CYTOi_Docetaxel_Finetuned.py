...
# subset shared samples and genes in TCGA expression and mutation datasets (Docetaxel)
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
        Clas = torch.load(
            save_models_to + 'Class_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Docetaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt')
