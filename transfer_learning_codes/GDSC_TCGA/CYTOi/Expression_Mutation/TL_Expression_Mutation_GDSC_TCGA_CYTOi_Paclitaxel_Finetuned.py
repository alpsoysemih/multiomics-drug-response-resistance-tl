...
save_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_TCGA/Expression_Mutation/"
save_finetuned_models_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_TCGA/Expression_Mutation/Finetuned_Models/"
save_results_to = "/arf/home/salpsoy/Thesis_Work/Results/GDSC_DEGs_inhibitors/CYTOi/save_models/GDSC_TCGA/Expression_Mutation/AUC and Cost Plots (Finetuned)/"
...
# subset shared genes and samples in GDSC expression, mutation, and response datasets (CYTOi)
...
# assign GDSC expression, mutation and response datasets (CYTOi) to new variables
exprs_z = GDSCE
mut = GDSCM
responses = GDSCR
...
# subset GDSC expression and mutation datasets (CYTOi) as to drugs
expression_zscores = []
mutations = []
for drug in drugs:
...
# filter shared samples between the subsetted GDSC expression and mutation datasets (CYTOi) 
ls5 = GDSCEv2.index.intersection(GDSCMv2.index)


# subset shared genes and samples in the subsetted GDSC expression, mutation, and response datasets (CYTOi) 
...
for iters in range(max_iter):
    ...
    for train_index, test_index in skf.split(GDSCEv2.values, Y):
        ...
        AutoencoderE = torch.load(
            save_models_to + 'Exprs_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
        AutoencoderM = torch.load(
            save_models_to + 'Mut_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
        Clas = torch.load(
            save_models_to + 'Class_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation.pt')
    ...
    torch.save(AutoencoderE,
               f"Exprs_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(
        AutoencoderM, f"Mut_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt")
    torch.save(Clas, f'Class_Paclitaxel_GDSC_TCGA_CYTOi_Expression_Mutation_Finetuned_{iters + 1}.pt')
